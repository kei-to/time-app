...
export class AppComponent {
  ...
  // elementTimeCompea(index: number){
  //   if(this.elements[index].flg){
  //     return;
  //   }
  //   const [hours, minutes] = this.elements[index].text.split(':').map(Number);
  //   let time: Date = new Date();
  //   time.setHours(hours, minutes, 0, 0);
  //   if( time < new Date ){
  //     this.elements[index].className = this.elements[index].className == "normal" ? "highlight":"normal";
  //     AppComponent.beep();
  //   }
  // }
  ...
  addElement() {
    ...
    const newElement: TimeElementModel = {
      label: `テキスト ${index}`,
      time: new Date(now.setHours(0, 0, 0, 0)), // 初期 00:00
  ...
  onSelectChange(index: number) {
    ...
    if (element.selectedMinutes != null) {
      ...
      element.time = future;
      console.log(`future:`, future);
      console.log(`element.selectedMinutes:`, element.selectedMinutes);
